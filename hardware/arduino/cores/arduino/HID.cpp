...
// 16 buttons, 4 Analog Axis
	...
	0xa1, 0x01,			// COLLECTION (Application)
        ...
        // X and Y Axis - 16 Bit Placeholders with 10bit values
    	0x05, 0x01,			// USAGE_PAGE (Generic Desktop)   
    	0x15, 0x00,			// LOGICAL_MINIMUM (0)
		...
		// 8 bit Throttle and Rudder - 16 Bit Placeholders with 10bit values
...
void Joystick_::setState(JoyState_t *joySt)
{
	...
	uint16_t yAxisTmp;
	uint16_t rudderTmp;
	uint16_t throttleTmp;
	buttonTmp = joySt->buttons;
	xAxisTmp = joySt->xAxis;
	yAxisTmp = joySt->yAxis;
	rudderTmp = joySt->rudder;
	throttleTmp = joySt->throttle;

	data[0] = buttonTmp & 0xFF;		// Break 16 bit button-state out into 2 bytes, to send over USB
	buttonTmp >>= 8;
	data[1] = buttonTmp & 0xFF;

	data[2] = xAxisTmp & 0xFF;		// X axis part 1
	xAxisTmp >>= 8;
	data[3] = xAxisTmp & 0xFF;	    // X axis part 0

    data[4] = yAxisTmp & 0xFF;      // Y axis part 1
    yAxisTmp >>= 8;
    data[5] = yAxisTmp & 0xFF;      // Y axis part 0
   
    data[6] = throttleTmp & 0xFF;   // throttle axis part 1
    throttleTmp >>= 8;
    data[7] = throttleTmp & 0xFF;   // throttle axis part 0
      
    data[8] = rudderTmp & 0xFF;      // rudder part 1
    rudderTmp >>= 8;
    data[9] = rudderTmp & 0xFF;      // rudder   part 0
